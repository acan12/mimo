...
public class MapOverlays extends ItemizedOverlay<OverlayItem> implements
	...
	public MapOverlays(Drawable defaultMarker, Context context,
			boolean setTouchedEnabled, boolean setTapMarkerEnabled) {
		super(boundCenterBottom(defaultMarker));
		this.mContext = context;
		this.isTouched = setTouchedEnabled;
	...
	public boolean onTap(final GeoPoint gp, MapView mapView) {
		if (isTap) {
			String coord = gp.getLatitudeE6() / 1E6 + "," + gp.getLongitudeE6()
					/ 1E6;
					...
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
							Intent i = new Intent(mContext,
									InputDetailActivity.class);
							p.println("----tap overlay----paramid:"
									+ idActivity);
							if (idActivity == 0) {
							...
							p.println("----from mapoverlay lat:"
									+ gp.getLatitudeE6() / 1E6);
							p.println("----from mapoverlay lon:"
...
//							mContext.startActivity(i);
	...
	// // Drawable drawable =
	// this.mContext.getResources().getDrawable(R.drawable.cheese);
	// // MapOverlays itemizedoverlay = new MapOverlays(drawable,
	// this.mContext);
	// // OverlayItem overlayitem = new OverlayItem(p, null,
	// "You are in Butcher Location.");
